...
### Phase 2: 팀 구성 및 태스크 생성

1. `TeamCreate`로 팀 생성 (team_name: `research-{slug}`)
   - Phase 1에서 선택된 N개 에이전트를 팀원으로 등록
2. `TaskCreate`로 태스크 생성:
   - 조사 태스크 N개 (선택된 전문가 각 1개)
   - 교차 검증 태스크 1개 (blockedBy: 조사 N개)
   - 종합 보고서 태스크 1개 (blockedBy: 교차 검증)
...
모든 에이전트 공통:
- `subagent_type: "general-purpose"` 사용 (WebSearch, WebFetch 필요)
- `run_in_background: true`로 병렬 실행
- `team_name` 지정하여 팀에 소속
- **교차 공유 의무**: 조사 중 다른 에이전트에게 유용한 정보 발견 시 `SendMessage`로 즉시 공유
...
- 전문가 풀에서 추가 에이전트 1~2개 실행
- 이미 투입된 전문가의 범위를 좁힌 보충 조사도 가능
...
- 선택된 에이전트는 반드시 **병렬** 실행 (단일 메시지에 N개 Task 호출)
