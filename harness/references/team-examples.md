...
> 리서치 에이전트는 커스텀 정의 없이 `general-purpose` + 상세 프롬프트로 동작한다. WebSearch/WebFetch가 필요하고 역할이 단발성이기 때문.
...
Phase 2: Phase 1 팀 종료 → prose-stylist를 새 팀원으로 추가 (또는 서브 에이전트로 호출)
         prose-stylist가 3개 산출물을 Read하여 집필
...
Phase 4: prose-stylist가 리뷰 결과 반영하여 최종 수정
```
