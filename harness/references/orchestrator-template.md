...
1. 팀 생성:
   ...
   TeamCreate(
     ...
     members: [
       { name: "{teammate-1}", agent_type: "{type}", prompt: "{역할 설명 및 작업 지시}" },
       { name: "{teammate-2}", agent_type: "{type}", prompt: "{역할 설명 및 작업 지시}" },
       ...
     ]
   )
   ```
...
2. 팀 정리 (TeamDelete 또는 리더가 clean up)
...
|---------|------|------|-------------------|
| {agent-1} | {입력 소스} | `_workspace/{phase}_{agent}_{artifact}.md` | true |
| {agent-2} | {입력 소스} | `_workspace/{phase}_{agent}_{artifact}.md` | true |
...
`~/.claude/skills/agent-research/skill.md`가 팬아웃/팬인 패턴의 실제 오케스트레이터 예시이다.
구조: 준비 → TeamCreate + TaskCreate → 4개 Agent 병렬 실행 → Read + 통합 → 정리.
