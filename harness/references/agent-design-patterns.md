...
```
에이전트가 2개 이상인가?
├── Yes → 에이전트 팀 (기본값 — 품질 우선)
│         에이전트 간 커뮤니케이션이 활발할수록 고품질 결과 산출.
│         독립적 작업이라도 교차 검증·발견 공유로 품질 향상.
│
└── No (1개) → 서브 에이전트
              단일 에이전트는 팀 구성 불필요.
```

> **핵심 원칙:** 서브 에이전트를 선택하려면 "왜 팀이 아닌가"에 대한 명확한 근거가 필요하다. 에이전트 팀에서 팀원들이 서로 도전하고 검증하는 커뮤니케이션 과정이 품질의 핵심 동력이다.
...
**원칙:** 에이전트 파일을 만들 때는 프롬프트만으로 부족한 이유가 있어야 한다. 단발성 작업이라면 `general-purpose` + 상세 프롬프트가 더 가볍다.
