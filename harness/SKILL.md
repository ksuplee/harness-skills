...
- 2개 이상의 에이전트가 작업할 때는 **항상 에이전트 팀을 우선 선택**한다
...
- 독립적으로 보이는 작업도 팀 모드에서 교차 검증·도전을 통해 품질이 향상된다

**서브 에이전트는 예외적 상황에서만 사용:**
- 에이전트가 1개뿐인 단발성 작업 (파일 탐색, 단순 코드 분석 등)
- 토큰 비용이 극도로 제한적이고 결과 품질보다 비용이 우선인 경우
- 에이전트 간 통신이 물리적으로 불가능한 구조 (예: 완전히 격리된 환경)

> **원칙:** 에이전트 팀이 기본이다. 서브 에이전트를 선택하려면 "왜 팀이 아닌가"에 대한 명확한 이유가 있어야 한다. 팀원들이 서로 도전하고 검증하는 과정에서 단독 작업으로는 도달할 수 없는 품질의 산출물이 나온다.
...
팀 전체를 조율하는 상위 스킬 생성. 구체적 템플릿은 `references/orchestrator-template.md` 참조.
